...
//	  V.3.48-B - Improved unit test report
//----------------------------------------------------------------------------- 

#ifndef FRAMEWORK_H
#define FRAMEWORK_H

#define FRAMEWORK_VER "3.48"
...
#pragma warning(pop)
		...
		public:
			...
			Test(const char* const pTestName, bool enab) noexcept
				:UnitSLink(),
				pName(pTestName),
				testFunc(nullptr),
				enabled(enab)
			...
			static void RunTests()
			{
				...
				while (pTmp != nullptr)
				{
					...
					assert(pTest);
					UnitTrace::out2("TEST: %s ", pTest->pName);
					if (pTest->enabled)
					{
				...
				UnitTrace::out2("\n");
				UnitTrace::out2(" Execution mode: %s\n", mode);
				UnitTrace::out2(" Tests Disabled: %d\n", unitStats.testDisabled);
				UnitTrace::out2(" Tests Passed: %d\n", unitStats.testPass);
				UnitTrace::out2(" Tests Failed: %d\n", unitStats.testFail);
				UnitTrace::out2("\n");
				UnitTrace::out2(" Test Count: %d\n", unitStats.testCount);
				UnitTrace::out2(" Total Checks: %d\n", unitStats.indvAsserts);
				UnitTrace::out2("\n");
				UnitTrace::out2("-----------------\n");
		...
		{ \
			...
			{ \
				...
				_UnitData.pSourceFilePath = __FILE__; \
				_UnitData.sourceLineNumber = __LINE__; \
				UnitTrace::out2("\n\tFAILED on equality check: '%s' not equal to '%s'\n", STRING_ME(value1), STRING_ME(value2) );\
				UnitTrace::out2("\t%s(%d): %s \n", _UnitData.pSourceFilePath, _UnitData.sourceLineNumber, _UnitData.pMemberName ); \
				return; \
			} \
			else\
		...
		{ \
			public: \
				...
				Test( STRING_ME(TestName##_##GroupName##_Test), TestFlagEnable) \
				{ \
				}; \
